...
document.addEventListener('DOMContentLoaded', function() {
    const cells = document.querySelectorAll('.td-calca'); 
    cells.forEach(cell => {
        cell.addEventListener('click', function() {
            const url = this.getAttribute('data-url');
            if (url) {
                window.location.href = url;
                console.log("okokok!winthegood!");
            }else{
                console.log("包iJFK了few了看军费我看见！");
            }
        });
    });
    ...
    const displayElement = document.getElementById('g26');
    ...
    if (storedValue) {
        ...
        // 4. 如果没有找到数据，显示一个提示信息
        displayElement.textContent = 'Press To Calculate';
        console.warn('LocalStorage 中未找到数据键:', dataKey);
    }

    const dataKey2 = 'calculated_result2';
    const displayElement2 = document.getElementById('g27');
    // 1. 从 localStorage 中读取数据
    const storedValue2 = localStorage.getItem(dataKey2);
    if (storedValue2) {
        displayElement2.textContent = storedValue2;
    } else {
        // 4. 如果没有找到数据，显示一个提示信息
        displayElement2.textContent = 'Press To Calculate';
        console.warn('LocalStorage 中未找到数据键:', dataKey2);
    }

    const dataKey3 = 'calculated_result3';
    const displayElement3 = document.getElementById('g28');
    // 1. 从 localStorage 中读取数据
    const storedValue3 = localStorage.getItem(dataKey3);
    if (storedValue3) {
        displayElement3.textContent = storedValue3;
    } else {
        // 4. 如果没有找到数据，显示一个提示信息
        displayElement3.textContent = 'Press To Calculate';
        console.warn('LocalStorage 中未找到数据键:', dataKey3);
    }
